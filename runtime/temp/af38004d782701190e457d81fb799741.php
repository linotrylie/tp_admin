<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:77:"D:\work\test\tp_model\public_html/../application/admin\view\upload\index.html";i:1539829330;s:78:"D:\work\test\tp_model\public_html/../application/admin\view\template\base.html";i:1488957233;s:89:"D:\work\test\tp_model\public_html/../application/admin\view\template\javascript_vars.html";i:1488957233;}*/ ?>
﻿<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
    ...
    <div id="tab_upload" class="HuiTab">
        ...
        <div class="tabCon">
            <div>
                ...
                <input id="fileupload" type="file" name="file[]" data-url="<?php echo \think\Url::build('upload'); ?>" multiple="multiple">
            </div>
        </div>
        <div class="tabCon">
...
<script>
    ...
    $(function () {
        ...
        $(".photo-list").on("click", '.photo-list-select', function () {
            img_arr.push($(this).parent().prev().attr('data-src'));
            $(this).removeClass('photo-list-select').addClass('photo-list-cancel');
            $(this).text('取消');
        });
        //取消已选则的图片
        $(".photo-list").on("click", '.photo-list-cancel', function () {
            console.log(img_arr.indexOf($(this).parent().prev().attr('data-src')));
            if (img_arr.indexOf($(this).parent().prev().attr('data-src')) >= 0) {
                img_arr.splice(img_arr.indexOf($(this).parent().prev().attr('data-src')), 1);
            }
            $(this).removeClass('photo-list-cancel').addClass('photo-list-select');
            $(this).text('选择');
        });
        //选择图片并返回地址
        $(".photo-list").on("click", '.photo-list-finish', function () {
            if (img_arr.indexOf($(this).parent().prev().attr('data-src')) == -1) {
                img_arr.push($(this).parent().prev().attr('data-src'));
            }          
            img_str = img_arr.join();
            // console.log(img_str);
            callback(callbackId, img_str);
        });
                ...
                if (ret.code) {
                    ...
                    for(var i in ret.data.list) {
                        ...
                        html += '<div class="photo-list-item">'
                                ...
                                + '<a class="photo-list-btn photo-list-select radius ml-10" href="javascript:;">选择</a><br><br>'
                                + '<a class="photo-list-btn photo-list-finish radius ml-10" href="javascript:;">选择完成并结束</a>'
                                + '</div>'
                                + '</div>';
                    }
