    ...
    */
    ...
    public function excel_import($file){
        ...
        // array(10) {
            ...
            //   ["J"] =&gt; string(12) "京东链接"
            // }
        $header = ['A' => 'name', 'B' => 'code', 'C' => 'type_id', 'D' => 'color', 'E' => 'texture', 'F' => 'size', 'G' => 'volume', 'H' => 'brand', 'I' => 'tm_url', 'J' => 'jd_url'];
        $result = \Excel::parse($file, $header, 1, function ($data) {
            /*
             * $data = [
            ...
            foreach ($data as &$v) {
                ...
                if ($v['tm_url']) {
                    $array['url_name'][] = '天猫'; 
                    $array['url_address'][] = $v['tm_url']; 
                }
                if ($v['jd_url']) {
                    $array['url_name'][] = '京东'; 
                    $array['url_address'][] = $v['jd_url']; 
                }
                if ($v['tm_url'] || $v['jd_url']) {
                    # code...
                    $v['url'] = json_encode(['url_name'=>$array['url_name'],'url_address'=>$array['url_address']]);
                }
                unset($v['tm_url']);
                unset($v['jd_url']);
                $v['isimport'] = 1;
            ...
            Db::name('product')->insert($data[0],true);
        });
        if ($result) {
            return true;
        }
