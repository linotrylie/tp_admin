    ...
    public function tagMenu($tag)
    {
        ...
        foreach ($menu as $k => $m) {
            ...
            switch ($m) {
                ...
                case 'show':
                case 'sshow':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '查看';
                    list($url, $param) = $this->parseUrl($url, 'id=$vo["id"]');
                    $parseStr .= ' <a title="' . $title . '" href="javascript:;" onclick="layer_open(\'' . $title . '\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" style="text-decoration:none" class="ml-5"><i class="Hui-iconfont">&#xe665;</i></a>';
                    break;
                case 'sdelete':
                ...
                case 'sdeleteforever':
                    ...
                    break;
                case 'excel': // zhitian add 生成excel
                case 'sexcel':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '导出';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a href="<?php echo \think\Url::build(\''.$url.'\', Input(\'param.\')); ?>" target="_blank" class="btn btn-secondary radius mr-5"><i class="Hui-iconfont">&#xe644;</i>' . $title . '</a>';
                    break;
                case 'import': // zhitian add 生成excel
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '导入';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a class="btn btn-primary radius mr-5" href="javascript:;" onclick="layer_open(\'' . $title . '\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')"><i class="Hui-iconfont">&#xe645;</i> ' . $title . '</a>';
                    break;
                default:
